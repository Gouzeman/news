...
<main class="flex-1">
    ...
    <div class="max-w-[800px] mx-auto ">
        <div class="w-full bg-white flex flex-col p-[24px] mb-[12px] border border-solid border-[#0000001a] rounded-[16px]">
            <h2 class="flex justify-center text-2xl font-semibold">Изменение новости</h2>
            <form action="../vendor/update.php" method="post" class="flex flex-col gap-5 mb-2" enctype="multipart/form-data">
                <!--------------- ТЕМА ---------------->
                <div class="flex flex-col gap-2">
                    <label for="" class="uppercase text-lg">Тема</label>
                    <input class="border border-[#ccc] border-solid text-base p-[8px]" placeholder="Тема новости" name="theme" value="<?= $news['theme'] ?>" maxlength="50">
                </div>
                <!------------- НАЗВАНИЕ НОВОСТИ ------------>
                <div class="flex flex-col gap-2">
                    <label for="" class="uppercase text-lg">название</label>
                    <input class="border border-[#ccc] border-solid text-base p-[8px]" placeholder="Название новости" name="title" value="<?= $news['title'] ?>" maxlength="50">
                </div>
                <!-------------ОПИСАНИЕ НОВОСТИ ------------->
                <div class="flex flex-col gap-2">
                    <label for="" class="uppercase text-lg">описание</label>
                    <input class="border border-[#ccc] border-solid text-base p-[8px]" placeholder="Короткое описание новости" name="description" value="<?= $news['description'] ?>" maxlength="50">
                </div>
                <!--------------- ТЕКСТ НОВОСТИ ---------------->
                <div class="flex flex-col ">
                    <label for="" class="uppercase text-lg">Текст новости</label>
                    <textarea class="border border-[#ccc] border-solid p-[10px] " placeholder="Текст новости" maxlength="6000" rows="10" name="text"><?= $news['text'] ?></textarea>
                </div>
                <!------------------- КАТЕГОРИИ ------------------>
                <div class="flex flex-col gap-2">
                    <label for="" class="uppercase text-lg">категории</label>
                    <input class="border border-[#ccc] border-solid text-base p-[8px]" placeholder="1 тег" name="category" value="<?= $news['category'] ?>" maxlength="50">
                </div>
                <input type="hidden" name="id" value="<?= $news['id'] ?>">
                <!----------- кнопка прикрепления файла ------------>
                <?php if($news['image']): ?>
                    <img src="../images/<?= $news['image'] ?>" class="w-[200px] rounded-lg mb-2">
                <?php endif; ?>
                ...
                <div class="flex justify-between gap-2">
                    ...
                    <button type="submit" style="width: 50%;" class="bg-[#1588e2] rounded-[10px] p-2 hover:opacity-[0.8] active:scale-[0.98]">
                        <span class="font-medium text-lg text-white ">Изменить</span>
                    </button>
                    <!------------ кнопка создания ------------>
                    <a href="../index.php" type="button" style="background-color: #ef4444; width: 50%;" class="rounded-[10px] p-2  text-center hover:opacity-[0.8] active:scale-[0.98]">
